...
<div id="print_area" style="page-break-after: always; margin-top: -25px;">
		...
		<div style="width:100%;overflow: hidden;border-width: 1px 1px 1px 1px !important; padding: 10px;">
			...
			<?php else : ?>
			    <?php if (isset($dupflag)): ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">DUPLICATE-Customer Copy</span>
				<?php elseif (isset($exception)): ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">EXCEPTION-Customer Copy</span>
				<?php else : ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">Customer Copy</span>
				<?php endif ?>
			<?php endif ?>
	...
	<div id="print_area" style="margin-top: 30px;">
		...
		<div style="width:100%;overflow: hidden;border-width: 1px 1px 1px 1px !important; padding: 10px;">
			...
			<?php else : ?>
			    <?php if (isset($dupflag)): ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">DUPLICATE-Office Copy</span>
				<?php elseif (isset($exception)): ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">EXCEPTION-Office Copy</span>
				<?php else : ?>
					<span style="text-align: center;width: 25%;display: block;float: left;">Office Copy</span>
				<?php endif ?>
			<?php endif ?>
			<span style="text-align: center;width: 55%;display: block;float: left;"><?php echo $nature; ?></span>
			<span style="width: 20%;display: block;float: left;">User : <?php echo $txn->user; ?></span>
...
<?php if (isset($dupflag)): ?>
	window.location.href =  "<?php echo $this->config->item('base_url');?>transaction/view?txncode=4&txntype=till&subtitle=FC SALES";
<?php else : ?>
	if ('<?php echo $txn->transactionType; ?>' == 'FCP') {
		...
		} else if ('<?php echo $txn->transactionType; ?>' == 'FCS') {
		...
		} else if ('<?php echo $txn->transactionType; ?>' == 'FCR') {
		...
		} else if ('<?php echo $txn->transactionType; ?>' == 'FCI') {
		...
		} else if ('<?php echo $txn->transactionType; ?>' == 'PFC') {
			window.location.href =  "<?php echo $this->config->item('base_url');?>transaction/create/pfcawithdraw/new?txncode=2&txntype=PFC&subtitle=PFCA WITHDRAWAL";
		} else {
