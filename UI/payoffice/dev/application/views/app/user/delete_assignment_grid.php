...
<table id="ap-data-table" class="ap-data-table" style="width:100%">
    ...
    <?php if ($item->USERLEVEL != 'Admin')  : ?>      
    <?php 

    $url = $this->config->item('base_url')."user/remove_assignment?operationtype=RMVASS&subtitle=REMOVE USER ASSIGNEMENT USER&usrPF=".$item->USERPF."&userName=".$item->USERNAME."&userLevel=".$item->USERLEVEL."&userTill=".$item->USERTILL."&effDate=".$item->EFFDATE;
    $row_class = 'ap-rw-def';    
