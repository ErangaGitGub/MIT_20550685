...
//        String effDate = new SimpleDateFormat("yyyyMMdd").format(new java.util.Date());
        ...
        //////
        try {
            String query1 = "SELECT s1.SCUSER as userId , "
                    + "s3.SCUNME as userName , "
                    + "s3. SCSTAT as userStatus , "
                    + "s2.SCBRN, "
                    + "TRIM(s1.SCGRPP) as profile "
                    + "FROM ISMS101.SCP01001 s1 "
                    + "left join "
                    + dataLibrary + ".SCP9006 s2 on  s1.SCUSER = s2.SCUSER "
                    // + "BOCPRODDTA" + ".SCP9006 s2 on  s1.SCUSER = s2.SCUSER "
                    + "inner join "
                    + "ISMS101.SCP001L5 s3 on s1.SCUSER = s3.SCUSER "
                    + "where "
                    + "s1.SCUSER like '%" + userID + "%' and s1.SCSLST = '" + environmrnt + "'"
                    + "and s3. SCSTAT = ''";
            try {
            ...
            while (rs.next()) {
                ...
                data.setBranchCode(rs.getString("SCBRN"));
                data.setName(rs.getString("userName"));
                data.setUserStatus(rs.getString("userStatus"));
        ...
        try {
            String query2 = "select max(ULDATE) as lastLogDate, max(ULTIME) as lastLogTime from  " + txnLibrary + ".pyp01101  where "
                    + " ULDATE = (select  max(ULDATE) from  " + txnLibrary + ".pyp01101  where  ULUSRID like '%" + userID + "%') ";
            try {
                con = DbConnection.getConnection();
            } catch (SQLException e) {
                logger.error("Error in create connection : ", e);
        ...
        try {
            String query3 = "select a.UALEVEL as userLevel, a.UAEFDATE as effectiveDate, a.UATILL as userTill, b.TILDESC as tillDesc, c.ULDESC as levelDesc "
                    + " from  " + txnLibrary + ".pyp01001 a, " + txnLibrary + ".pyp00401 b, " + txnLibrary + ".pyp00801 c "
...
//////////////////////
    ...
    public boolean logUserData(String userID, String ip, String mName) {
        ...
        try {
            ...
            String query = "INSERT INTO " + txnLibrary + ".PYP01101(ULUSRID,ULIPADD,ULHOST,"
    ...
    public String getBranchUserList() {
        ...
        String response = "";
        try {
            String query = "select SCUSER as USERPF "
                    + "from " + dataLibrary + ".SCP9006 ";
            try {
...
public String getEffectiveDate() {
    ...
    public String getBranchUserData(String pfNumber) {
        ...
        try {
            String query = "select SCUNME as USERNAME "
                    + "from ISMS101.SCP001L5 "
                    + "where SCUSER = '" + pfNumber + "'";
            try {
                con = DbConnection.getConnection();
            } catch (SQLException e) {
                logger.error("Error in create connection : ", e);
        ...
        try {
            if (checkDuplicatePf(txn.getPfNumber()) == false) {
                try {
                    ...
                    String query = "INSERT INTO " + txnLibrary + ".PYP00901(UMUSRPF,UMUSRNM,"
                            + "UMUSRST,UMENDAT,UMENUSR,UMENIP,UMENHOST"
                            + ") values ('" + txn.getPfNumber() + "','" + txn.getName() + "'"
                            + ",'" + 'A' + "','" + timeStamp + "'"
...
public String assignUserData(UserData usr) {
        ...
        if ((alreadyAssignUser(usr) == false)&&(alreadyAssignRole(usr)==false)) {
            try {
                String query = "INSERT INTO " + txnLibrary + ".PYP01001 "
    ...
    public String removeUserData(UserData usr) {
        ...
        if (checkLiveStatus(usr) == false) {
            try {
                String query = "UPDATE " + txnLibrary + ".PYP00901 "
    ...
    public String removeUserAssignData(String pfnumber, String userlevel, String usertill, String effdate) {
        ...
        try {
            String queryOne = "select TILIDNO "
                    + "from " + txnLibrary + ".PYP00401 where TILDESC= '" + usertill + "' ";
            try {
                con = DbConnection.getConnection();
            } catch (SQLException e) {
                logger.error("Error in create connection : ", e);
        ...
        try {
            String query = "select TPCURBAL,TPCURID "
                    + "from " + txnLibrary + ".PYT00601 where TPTILID = '" + tillNo + "' ";
            try {
                con = DbConnection.getConnection();
            } catch (SQLException e) {
                logger.error("Error in create connection : ", e);
            ...
            if ((j == 0)||(tillNo==1)) {
                try {
                    String newQuery = "UPDATE " + txnLibrary + ".PYP01001 "
    ...
    public String getDailyAssignments() {
        ...
        try {
            String query = "select UAUSRPF as USERPF,"
                    ...
                    + "from " + txnLibrary + ".PYP01001 a,"
                    +  txnLibrary + ".PYP00801 b,"
                    +  txnLibrary + ".PYP00401 c "
    ...
    public String getUserAssignmentList() {
        ...
        try {
            String query = "select UAUSRPF,UAUSRNM,UALEVEL,UATILL "
                    + "from " + txnLibrary + ".PYP01001 where UAEFDATE = '" + currentDate + "' ";
                GetTableData tableData = new GetTableData();
                response = tableData.getDBData(query);
            } catch (Exception e) {
