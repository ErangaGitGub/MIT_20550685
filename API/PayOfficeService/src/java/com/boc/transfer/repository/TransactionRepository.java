...
public class TransactionRepository {
    ...
    public String getCurrencyData() {
        ...
        String response = "";
        try {
            String query = "select CURSHRT,CURDESC  "
                    + "from PYP00201 where CURSHRT <> 'LKR'";
            try {
            ...
            logger.error("Error in load Currency details : ", e);
        }
        return response;
    }
...
//            } else {
            ...
            query = "select RPBUYRT as buyRate,RPSELRT as sellRate, RPCLING as ceiling,RPCURPET as currShrt from "
                    + " PYP00701 where RPSTAT = 'A' and RPCURPET = '" + shortcode + "' and "
                    + " RPDATE = '" + currentDate + "' and RPCRTTIM = (select  max(RPCRTTIM) from  pyp00701)";
...
//                            + " TRCRTUSR, TRIPADDR, TRHOSTNM, TRTIMEST, TRREFNUM, TRREMARK, TRREFPRV, "
//                            + " TRNATXCD, TRACTYCD, TRTXSECD, TRITRSCD, TRISFXCV, TRCOUNTR, "
//                            + " TRCURR1, TRAMT1, TREXRAT1, TRCVAMT1, TRINAMT1, TRUSDRT1, TRUSDEQ1, "
//                            + " TRCURR2, TRAMT2, TREXRAT2, TRCVAMT2, TRINAMT2, TRUSDRT2, TRUSDEQ2, "
//                            + " TRCURR3, TRAMT3, TREXRAT3, TRCVAMT3, TRINAMT3, TRUSDRT3, TRUSDEQ3, "
//                            + " TRCURR4, TRAMT4, TREXRAT4, TRCVAMT4, TRINAMT4, TRUSDRT4, TRUSDEQ4, "
...
//                }
//
//                resp = withinTill(query, txn.getTransactionType(), txn.getUserTill(), txn.getIcurrencyselector1(), txn.getIcurrencyselector2(), txn.getIcurrencyselector3(), txn.getIcurrencyselector4(), txn.getTamount1(), txn.getTamount2(), txn.getTamount3(), txn.getTamount4(), txn.getTotalToCustomer(), txn.getUser(), txn.getIpAddress(), txn.getHostname());
//                try {
//                    if (!resp) {
//                        jSONObject.put("errorStatus", true);
//                        jSONObject.put("bankReference", "ERROR");
//                        jSONObject.put("errorMessage", "Error in Save Transaction Data.");
//                    } else {
...
//                    String s = commR.executeFCITransaction(txn.getAccountNo(), accType, toAccount, toAcctType, txn.getCamount1(), totalComm, txn.getRemarks(), txn.getReferenceNo());
//                    jSONObject.put("communicatorResult", s);
//                } else if (txn.getTransactionType().equalsIgnoreCase("PFC")) {
//                    accType = checkAccType(txn.getAccountNo());
//                    CommunicatorRepository commR = new CommunicatorRepository();
//                    // totComm= (Double.parseDouble(txn.getCommissionAmount())+Double.parseDouble(txn.getCeilingOrFloorCommission()));
//                    // String totalComm= String.valueOf(totComm);
//                    String s = commR.executePFCTransaction(txn.getAccountNo(), accType, toAccount, toAcctType, txn.getTamount1(), txn.getIcurrencyselector1(), txn.getRemarks(), txn.getReferenceNo());
//                    jSONObject.put("communicatorResult", s);
//
//                }
//            }//end of if else
            ...
            if (checkTillAmount(txn.getUserTill(), txn.getIcurrencyselector1(), txn.getIcurrencyselector2(), txn.getIcurrencyselector3(), txn.getIcurrencyselector4(), txn.getTamount1(), txn.getTamount2(), txn.getTamount3(), txn.getTamount4(), txn.getTransactionType(), txn.getTotalLKR()) == true) {
                try {
                    query = "INSERT INTO PYP00301 "
                            ...
                            + " TRCOMPER, TRCOMAMT, TRLKRTOT, TRINCAMT, TRCOMFCA, TRCUSTOT,TRRECAMT, TRREFAMT, "
                            + " TRBENAME, TRADDRES, TRBECNTR, TRUSRTIL, TRAIRTKT, TRBERMBK ) "
                            + " values ("
                            + txn.getUincode() + ", "
                            + getManDateFormatted() + ", '"
                            + txn.getTransactionType() + "', '"
...
//                }
    ...
    public boolean withinTill(String sql, String txnType, String till, String curr1, String curr2, String curr3, String curr4, String currAmount1, String currAmount2, String currAmount3, String currAmount4, String lkrAmount, String user, String ip, String host) {
                ...
                case "FCP":
                    try {
                        for (int i = 0; i < 4; i++) {
                            ...
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL + '" + currAmount + "'  where TPTILID = '" + till + "' and TPCURID = '" + curr + "' ";
                            stmt.addBatch(query);

///
                            query = "INSERT INTO PYP00501 "
                                    + "(TCSRCTIL, TCDESTIL, TCCURID, TCTRAMNT, TCCRTUSR, TCCRTIP, TCCRTHOST,"
                                    + " TCCRTTIM ) "
                                    + " values ('"
                                    + till + "', '"
                                    + till + "', '"
                        ...
                        query = "Update PYT00601  set TPCURBAL = TPCURBAL - '" + lkrAmount + "'  where TPTILID = '" + till + "' and TPCURID = 'LKR' ";
                        stmt.addBatch(query);
...
///
    ...
    public String getFilteredTransaction(String fromdate, String todate, String txntype, String txnststaus) {
        GetDataLibrary library = new GetDataLibrary();
        txnLibrary = library.loadTxnLibraty();
        String response = "";
        String whereClause = generateAllTxnListWhereClause(fromdate, todate,txntype,txnststaus);
    ...
    private String generateAllTxnListWhereClause(String fromdate, String todate, String txntype, String txnstatus) {
        String whereClause = "";
        String fromdateformatted = fromdate.replaceAll("-", "");
        String todateformatted = todate.replaceAll("-", "");
        
        
        if (txntype.equals("ALL") && txnstatus.equals("ALL")  ) {
              whereClause = " WHERE TREFDATE >= '"+ fromdateformatted +"' and TREFDATE <= '"+ todateformatted +"'";
        }else if (txntype.equals("ALL") && !"ALL".equals(txnstatus)){
              whereClause = "WHERE TREFDATE >= '"+ fromdateformatted +"' and TREFDATE <= '"+ todateformatted +"' and TRSTATUS = '"+ txnstatus +"' ";
        }else if (!"ALL".equals(txntype) && txnstatus.equals("ALL")){
              whereClause = "WHERE TREFDATE >= '"+ fromdateformatted +"' and TREFDATE <= '"+ todateformatted +"' and TRTXNTYP = '"+ txntype +"' ";
        } else {
              whereClause = "WHERE TREFDATE >= '"+ fromdateformatted +"' and TREFDATE <= '"+ todateformatted +"' and TRTXNTYP = '"+ txntype +"' and TRSTATUS = '"+ txnstatus +"' ";
        }
...
//                txnView.setForm12No(rs.getString("form12RefNo"));
                ...
                txnView.setAirTicketNo(rs.getString("airticketno"));
...
//                txnView.setCustaddr2(rs.getString("addressLine2"));
//                txnView.setAirTicketNo(rs.getString("airticketno"));
        ...
        try {
            ...
            switch (txnType) {
                case "FCS":
                    ...
                    query = "select TFMAXVAL,TFCOMPER, TFCOMVAL from PYP01201 where TFTXNTYP = 'FCS' and TFPASTYP = '" + passHolType + "' and TFSTATUS = 'A' ";
                ...
                case "FCP":
                    ...
                    break;
                case "FCR":
                    query = "select TFMAXVAL,TFCOMPER, TFCOMVAL from PYP01201 where TFTXNTYP = 'FCR' and TFSTATUS = 'A' ";
                    response = commissionCalc(query, txnType, dAmount);
                    break;
                case "FCI":
                    query = "select TFMAXVAL,TFCOMPER, TFCOMVAL from PYP01201 where TFTXNTYP = 'FCI' and TFPASTYP = '" + passHolType + "' and TFSTATUS = 'A' ";
                    response = commissionCalc(query, txnType, dAmount);
                    break;
            }
                ...
                switch (txnType) {
                    case "FCP":
                        try {
                            ...
                            cancelresp = false;    
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL - '" + currAmount1 + "'  where TPTILID = '" + till + "' and TPCURID = '" + curr1 + "' ";
                            stmt.addBatch(query);
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL - '" + currAmount2 + "'  where TPTILID = '" + till + "' and TPCURID = '" + curr2 + "' ";
                            stmt.addBatch(query);
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL - '" + currAmount3 + "'  where TPTILID = '" + till + "' and TPCURID = '" + curr3 + "' ";
                            stmt.addBatch(query);
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL - '" + currAmount4 + "'  where TPTILID = '" + till + "' and TPCURID = '" + curr4 + "' ";
                            stmt.addBatch(query);
                            query = "Update PYP00501  set TCSTATUS =  'C'  where  TCREFNUM = '" + txnRef + "' ";
                            stmt.addBatch(query);
                            query = "Update PYT00601  set TPCURBAL = TPCURBAL + '" + lkrAmount + "'  where TPTILID = '" + till + "' and TPCURID = 'LKR' ";
                            stmt.addBatch(query);
                            query = "update PYP00301  set TRSTATUS = 'C', TRAUTUSR='" + user + "', TRAUTRES = '" + reason + "', TRAUTTIM = '" + getCurrentTimeStamp() + "'  where TRREFNUM = '" + txnRef.replaceAll("\\s+", "") + "'";
                            stmt.addBatch(query);
                            }
