...
public class TranasactionController {
   ...
   @POST
    ...
    @POST
    @Path("getFilteredTransaction/")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String getFilteredTransaction(@HeaderParam("apiKey") String key, String txnTypeJson) {
        String response = "";
        ApiAuthontication authontication = new ApiAuthontication();
        JSONObject jSONObject = new JSONObject();
        JSONArray jSONArray = new JSONArray();
        try {
            if (!authontication.isUserAuthenticated(key)) {
                ...
                if (!jsonObj.has("fromdate")) {
                    jSONObject.put("errorStatus", true);
                    jSONObject.put("errorMessage", "From Date is required.");
                    return jSONObject.toString();
                }else if (!jsonObj.has("todate")) {
                    jSONObject.put("errorStatus", true);
                    jSONObject.put("errorMessage", "To Date is required.");
                    return jSONObject.toString();
                }else if (!jsonObj.has("txntype")) {
                    jSONObject.put("errorStatus", true);
                    jSONObject.put("errorMessage", "Transaction Type is required.");
                    return jSONObject.toString();
                }else if (!jsonObj.has("txnstatus")) {
                    jSONObject.put("errorStatus", true);
                    jSONObject.put("errorMessage", "Transaction Status is required.");
                    return jSONObject.toString();
                }else {   
                TransactionHandler handler = new TransactionHandler();
                response = handler.getFilteredTransaction(jsonObj.getString("fromdate"), jsonObj.getString("todate"),jsonObj.getString("txntype"),jsonObj.getString("txnstatus"));
            ...
            logger.error("Error in getFilteredTransaction : ", e);
        }
        return response;
    }
