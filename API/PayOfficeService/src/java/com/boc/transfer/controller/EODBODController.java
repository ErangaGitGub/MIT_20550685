...
//    }
    ...
    @POST
    @Path("startPreCheckOperation/")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String startPreCheckOperation(@HeaderParam("apiKey") String key, String dataJson) {
        String response = "";
        ApiAuthontication authontication = new ApiAuthontication();
        JSONObject jSONObject = new JSONObject();
        try {
            if (!authontication.isUserAuthenticated(key)) {
                ...
                if (!jsonObj.has("user")) {
                    ...
                    EODBODHandler handler = new EODBODHandler();
                    String user = jsonObj.getString("user");
                    response = handler.startPreCheckOperation(user);
                }
            }
        } catch (Exception e) {
            logger.error("Error in initial data insert : ", e);
    ...
    @POST
    @Path("startDayEndOperation/")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String startDayEndOperation(@HeaderParam("apiKey") String key, String dataJson) {
        String response = "";
        ApiAuthontication authontication = new ApiAuthontication();
        JSONObject jSONObject = new JSONObject();
        try {
            if (!authontication.isUserAuthenticated(key)) {
                ...
                if (!jsonObj.has("user")) {
                    ...
                    EODBODHandler handler = new EODBODHandler();
                    String user = jsonObj.getString("user");
                    response = handler.startDayEndOperation(user);
                }
            }
        } catch (Exception e) {
            logger.error("Error in startDayEndOperation: ", e);
        }
        return response;
    }
...
@POST
    @Path("getDailySummaryData/")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String getDailySummaryData(@HeaderParam("apiKey") String key) {
        logger.debug("getDailySummaryData");
        ApiAuthontication authontication = new ApiAuthontication();
        JSONObject jSONObject = new JSONObject();
        String summaryData = "";
        try {
            if (!authontication.isUserAuthenticated(key)) {
                ...
                return jSONObject.toString();
                    ...
                    EODBODHandler handler = new EODBODHandler();
                    summaryData = handler.getDailySummaryData();               
            }
        } catch (Exception ex) {
