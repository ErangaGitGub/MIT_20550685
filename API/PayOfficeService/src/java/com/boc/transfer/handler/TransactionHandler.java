...
public class TransactionHandler {
    ...
    public String getFilteredTransaction(String fromdate, String todate, String txntype, String txnstatus) {
        TransactionRepository repository = new TransactionRepository();
        String response = "";
        try {
            response = repository.getFilteredTransaction(fromdate, todate, txntype, txnstatus);
        } catch (Exception ex) {
            logger.error("Error in get Transactions data : ", ex);
        }
        return response;
...
//            txn.setPassportNo(formJson.getString("passportno"));
 ...
 //           txn.setUserTill(formJson.getString("userTill"));
            ...
            if (formJson.has("refundAmount") && !formJson.getString("refundAmount").equals("")) {
                txn.setRefundAmount(formJson.getString("refundAmount"));
            } else {
